...
- **Loop Control:** The loop breaks immediately on `Final Answer:`, `Action: None`, or any response with neither — preventing infinite loops from ambiguous model outputs.

## 2. Tool Layer: Pydantic & Pandas
- **Pydantic for State:** `ExcelTools` uses a Pydantic `BaseModel` for type-safe file path management and a clean declarative structure.
...
- **Data Cleaning on Load:** Currency columns (Price, Budget, Revenue, Amount) are stripped of `$` and `,` and cast to float on every load. Integer columns (Impressions, Clicks, Conversions) are similarly cleaned. This ensures numeric comparisons always work regardless of how the Excel file was originally formatted.
...
- **Chained Confirmations:** For multi-delete operations (e.g., "delete the 3 worst campaigns"), `chat()` accepts an optional `user_input=None` call to resume the ReAct loop after each confirmation without re-appending user input to history.
...
- **`eval()` Risk:** Using `eval()` to dispatch tool calls is convenient but carries security risk if the LLM produces unexpected code. In production this would be replaced with explicit argument parsers per tool.
